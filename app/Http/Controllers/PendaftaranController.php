...
class PendaftaranController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'alamat'            => 'required|string', // wajib, disimpan ke tabel siswas
            ...
            'kk'        => 'nullable|file|mimes:pdf,jpg,jpeg,png|max:20480',
            'akta'      => 'nullable|file|mimes:pdf,jpg,jpeg,png|max:20480',
        ...
        try {
            ...
            // ===== HUBUNGAN WALI: simpan langsung value dari form =====
            // DB enum: bapak, ibu, saudara_kandung, saudara_keluarga
            $hubunganMap = [
                'bapak'            => 'bapak',
                'ibu'              => 'ibu',
            ...
            // ===== KIRIM EMAIL =====
            // Kumpulkan email admin dari ENV (ADMIN_EMAIL)
            ...
            try {
                set_time_limit(60);
                // Email ke siswa & wali
                $emailSiswa = array_filter(array_unique(array_merge([$validated['email']], $emailWaliList)));
                $this->kirimEmailPendaftaran($pendaftaran, $siswa, $emailSiswa, false);
                // Email notifikasi ke admin sekolah
                if (!empty($adminEmails)) {
                    $this->kirimEmailPendaftaran($pendaftaran, $siswa, $adminEmails, true);
                }
            } catch (\Throwable $e) {
    ...
    private function kirimEmailPendaftaran(Pendaftaran $pendaftaran, Siswa $siswa, array $emailTargets, bool $isAdmin = false): void
    {
        ...
        $subject = $isAdmin
            ? "[PPDB BARU] {$namaSiswa} — {$sekolahNama} ({$kodeRegis})"
            : "Konfirmasi Pendaftaran PPDB - {$kodeRegis}";

        // Email siswa: info pendaftaran detail
        // Email admin: ringkasan singkat + link ke admin panel
        if ($isAdmin) {
            $adminPanelUrl = config('app.url') . '/admin/pendaftarans';
            $body = "<!DOCTYPE html><html lang='id'><head><meta charset='UTF-8'>
            <style>body{font-family:Arial,sans-serif;background:#f4f4f4;margin:0;padding:0}.wrap{max-width:600px;margin:30px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.1)}.hdr{background:#0f4c3a;color:#fff;padding:24px 28px;text-align:center}.hdr h1{margin:0;font-size:18px}.body{padding:24px 28px}.kode{font-size:22px;font-weight:700;font-family:monospace;color:#0f4c3a;background:#f0fdf4;padding:10px 18px;border-radius:8px;display:inline-block;margin:10px 0}.row{display:flex;margin:6px 0;font-size:13px}.lbl{color:#666;width:130px;flex-shrink:0}.val{font-weight:600;color:#111}.btn{display:inline-block;background:#0f4c3a;color:#fff;text-decoration:none;padding:11px 26px;border-radius:8px;font-weight:700;font-size:13px;margin-top:16px}.ftr{background:#f9fafb;border-top:1px solid #eee;padding:14px 28px;text-align:center;color:#999;font-size:11px}</style></head>
            <body><div class='wrap'>
            <div class='hdr'><h1>📋 Pendaftaran Baru Masuk</h1><p style='opacity:.8;margin:4px 0 0;font-size:13px'>PPDB Online — Notifikasi Admin</p></div>
            <div class='body'>
            <p style='font-size:13px;margin:0 0 12px'>Ada pendaftaran baru yang masuk dan perlu diverifikasi:</p>
            <div class='kode'>{$kodeRegis}</div>
            <div style='background:#f9fafb;border-radius:8px;padding:14px;margin:12px 0'>
            ...
            <a href='{$adminPanelUrl}' class='btn' style='color:#fff !important'>🔍 Buka Panel Admin →</a>
            </div>
            <div class='ftr'>Email ini dikirim otomatis. Hanya admin yang menerima email ini.</div>
            </div></body></html>";
        } else {
        $body = "<!DOCTYPE html>
        <html lang='id'>
