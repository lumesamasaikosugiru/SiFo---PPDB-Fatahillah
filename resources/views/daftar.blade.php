          ...
          <div id="wali-container" class="space-y-6">
            ...
            <div class="wali-item bg-gray-50 border border-gray-200 rounded-2xl p-5" data-index="0">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                  <label class="block text-xs font-semibold text-gray-600 mb-1.5">Alamat <span class="text-gray-400 font-normal">(Opsional)</span></label>
                  <textarea name="wali[0][alamat]" placeholder="Alamat orang tua/wali" rows="2"
                            class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-300 bg-white transition-all resize-none"></textarea>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
        <div class="bg-gradient-to-r from-teal-50 to-teal-100 px-8 py-6 border-b border-teal-100">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="font-bold text-gray-900 text-lg">Upload Dokumen Persyaratan</h2>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            @php
              $dokumenList = [
                ...
                ['name'=>'ijazah',   'label'=>'Ijazah / SKL',   'icon'=>'🎓', 'desc'=>'Ijazah atau SKL dari sekolah asal','required'=>true],
                ['name'=>'skhun',    'label'=>'SKHUN',          'icon'=>'📄', 'desc'=>'Surat Keterangan Hasil Ujian',      'required'=>true],
                ['name'=>'stl',      'label'=>'STL',            'icon'=>'📋', 'desc'=>'Surat Tanda Lulus',                'required'=>true],
              ];
            @endphp
...
<script>
  ...
  function validateStep(step) {
    ...
    if (step === 5) {
      const required = ['pas_foto'];
      required.forEach(name => {
        const f = document.getElementById('file-' + name);
        if (!f.files || !f.files.length) { Swal.fire({ icon:'warning', title:'Dokumen Wajib', text: 'Pas Foto wajib diupload.' }); valid = false; }
      });
  ...
  function addWali() {
    ...
    div.innerHTML = `
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="md:col-span-2">
          <label class="block text-xs font-semibold text-gray-600 mb-1.5">Alamat <span class="text-gray-400 font-normal">(Opsional)</span></label>
          <textarea name="wali[${idx}][alamat]" placeholder="Alamat orang tua/wali" rows="2" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-300 bg-white resize-none"></textarea>
        </div>
      </div>`;
  ...
  function saveCache() {
    ...
    document.querySelectorAll('.wali-item').forEach((item) => {
      waliData.push({
        ...
        alamat:     item.querySelector('textarea[name*="[alamat]"]')?.value || '',
      });
    });
    ...
    try {
      ...
      if (d.wali && Array.isArray(d.wali) && d.wali.length > 0) {
        ...
        if (firstItem && d.wali[0]) {
          ...
          const alamatEl = firstItem.querySelector('textarea[name*="[alamat]"]');
          if (alamatEl && w.alamat) alamatEl.value = w.alamat;
        }
        ...
        for (let i = 1; i < d.wali.length; i++) {
          ...
          if (emailEl && w.email) emailEl.value = w.email;
          const alamatEl = items[items.length-1].querySelector('textarea[name*="[alamat]"]');
          if (alamatEl && w.alamat) alamatEl.value = w.alamat;
        }
      }
    } catch(e) {}
        ...
        if (raw) {
          ...
          if (d.wali && Array.isArray(d.wali) && d.wali.length > 0) {
            ...
            if (firstItem && d.wali[0]) {
              ...
              const a = firstItem.querySelector('textarea[name*="[alamat]"]'); if (a && w.alamat) a.value = w.alamat;
            }
            for (let i = 1; i < d.wali.length; i++) {
              addWali();
              const items = container.querySelectorAll('.wali-item');
              ...
              const a2 = item.querySelector('textarea[name*="[alamat]"]'); if (a2 && w.alamat) a2.value = w.alamat;
            }
          }
        }
      } catch(e) {}
